...
<?php
		...
		try{
			...
			<div class="commentator-social-message">
				<div>
					<h1><?php _e("Error: please try again!" ); ?></h1>
					<h4><?php _e("Original error message: "); ?> <?php echo $e->getMessage(); ?></h4>
				</div>
			</div>
			<?php
		}
	...
	?>
			<div class="commentator-social-message">
				<div>
					...
					<h4><?php _e('You can now join the conversation !'); ?></h4>
				</div>
			</div>
...
function commentator_comment( $comment, $args, $depth ) {
	...
	$voteAuthorID = $current_user->ID;
	$avatar = ($comment->comment_author_email) ? get_avatar( $comment->comment_author_email ) : get_avatar( $comment );
	$user_profile = $_SESSION["commentator_user_profile"];
	if(!is_user_logged_in() && $user_profile != null){
		$voteAuthorID = $_SESSION["commentator_provider"]."-".$user_profile['identifier'];
	}
	...
	$commentUrl = (isset($userpro) && $comment_user->ID) ? $userpro->permalink( $comment_user->ID ) : get_comment_author_url();
	$commentUrl = empty($commentUrl) ? "#comment-".get_comment_ID() : $commentUrl;

	$authorLink = (isset($userpro) && $comment_user->ID) ? '<a href="'.$userpro->permalink( $comment_user->ID ).'">'.userpro_profile_data('display_name', $comment_user->ID) . '</a>' : get_comment_author_link();
	?>
		...
		<article id="comment-<?php comment_ID(); ?>" class="commentator-comment-content">
	        ...
	        <div class="commentator-comment-body">
	        	<div class="commentator-comment-header">
                    ...
                    <?php if ( $comment_user->ID == $post->post_author ){ ?>
                    ...
                    <?php if(isset($userpro) && $comment_user->ID){ ?>
                    <!-- <div class="commentator-user-pro-badges">
                    </div> -->
                    <?php } ?>
...
function commentator_scripts() {
	wp_enqueue_script( 'commentator-script', plugins_url( '/js/commentator-script.js', __FILE__ ), array( 'jquery' ), '1.3', true);
